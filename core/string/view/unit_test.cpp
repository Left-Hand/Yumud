#include "null_terminated_uchars_view.hpp"
...
[[maybe_unused]]static void test_null_terminated(){
    ...
    // 测试最大长度限制
    {
        ...
        static_assert(ustr.serialized_length() == 11);
    }
    ...
    // 超出最大长度的情况
    {
        ...
        constexpr auto uchars = std::span(bytes);
        // 应该截断到最大长度
        constexpr auto ustr = NullTerminatedUCharsView<10>::from_uchars_bounded(uchars);
        static_assert(ustr.length() == 10);  // 截断到最大长度
    }

    {
        constexpr NullTerminatedUCharsView<10> ustr = NullTerminatedUCharsView<10>{.p_uchars_ = nullptr, .length_ = 0};
        static_assert(ustr.length() == 0);
    }

    {
        ...
        static_assert(ustr.serialized_length() == 6);  // 包含终止符
    }
