...
struct type :public Reg16<>{static constexpr RegAddress address = addr; uint16_t data;} DEF_R16(name)

#define REG8_QUICK_DEF(addr, type, name)\
struct type :public Reg8<>{static constexpr RegAddress address = addr; uint8_t data;} DEF_R8(name)
