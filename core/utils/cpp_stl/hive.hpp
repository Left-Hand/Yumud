// Copyright (c) 2025, Matthew Bentley (mattreecebentley@gmail.com) www.stdlib.org
...
namespace std
{
	...
	#ifndef std_FROM_RANGE // To ensure interoperability with other std lib containers
		#define std_FROM_RANGE
...
namespace std
{
	template <std::hive_iterator_concept it_type, typename distance_type>
	...
	template <std::hive_iterator_concept it_type>
...
public:
	...
	hive(std::ranges::from_range_t, range_type &&rg, const hive_limits block_limits, const allocator_type &alloc = allocator_type()):
		allocator_type(alloc),
		erasure_groups_head(nullptr),
		unused_groups_head(nullptr),
		total_size(0),
	...
	template<class range_type>
		requires std::ranges::range<range_type>
	hive(std::ranges::from_range_t, range_type &&rg, const allocator_type &alloc = allocator_type()):
		hive(std::ranges::from_range, std::move(rg), block_capacity_default_limits(), alloc)
	{}
...
private:
	...
	{
		hive temp(std::hive_limits(new_min, new_max));
...
public:
	...
	void reshape(const std::hive_limits block_limits)
	{
		check_capacities_conformance(block_limits);
...
namespace std
{
	...
	void swap (std::hive<element_type, allocator_type> &a, std::hive<element_type, allocator_type> &b) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_swap::value || std::allocator_traits<allocator_type>::is_always_equal::value)
	{
		a.swap(b);
	}
	...
	typename std::hive<element_type, allocator_type>::size_type erase_if(std::hive<element_type, allocator_type> &container, predicate_function predicate)
	{
		typedef typename std::hive<element_type, allocator_type> hive;
	...
	template <class element_type, class allocator_type>
	typename std::hive<element_type, allocator_type>::size_type erase(std::hive<element_type, allocator_type> &container, const element_type &value)
	{
		return erase_if(container, std::hive_eq_to<element_type>(value));
	}
