...
static constexpr size_t tuple_element_offset_v = tmp::
    details::_tuple_element_offset_v<I, Tup>::value;

template<typename ... Args>
static constexpr size_t total_bytes_of_args_v = tmp::details::
    _total_bytes_of_args_v<sizeof...(Args) - 1, Args...>::value;

template<typename Tup>
static constexpr size_t total_bytes_of_packed_tuple_v = tmp::details::
...
template<
    typename Tup,
    size_t N = tmp::total_bytes_of_packed_tuple_v<std::decay_t<Tup>>
>
__inline constexpr
...
template<
    ...
    size_t TupSize = tmp::total_bytes_of_args_v<Tup>,
    size_t ArgSize = sizeof(Arg)
>
__inline constexpr 
...
template<typename Tup, size_t N = tmp::total_bytes_of_args_v<std::decay_t<Tup>>>
__inline constexpr 
...
template<
    ...
    typename Ret,
    typename ArgsTup,
    size_t N = tmp::total_bytes_of_packed_tuple_v<ArgsTup>
>
__inline constexpr 
...
template<
    ...
    typename Ret,
    typename ... Args,
    size_t N = tmp::total_bytes_of_packed_tuple_v<std::tuple<Args...>>
>
__inline constexpr 
...
constexpr Ret invoke_func_by_serialzed_bytes(Ret(*fn)(Args...), const std::span<const uint8_t, N> bytes){
    ...
    return tmp::details::_invoke_func_by_serialzed_bytes_impl
...
[[nodiscard]] constexpr Ret invoke_func_by_serialzed_bytes(
    auto & obj, 
    Ret(std::remove_reference_t<decltype(obj)>::*fn)(Args...), 
    const std::span<const uint8_t, N> bytes
){
    if constexpr(std::is_void_v<Ret>) 
        tmp::details::_invoke_memfunc_by_bytes_impl<std::remove_reference_t<decltype(obj)>, Ret, Args...>(obj, fn, bytes);
    else 
        return tmp::details::_invoke_memfunc_by_bytes_impl<std::remove_reference_t<decltype(obj)>, Ret, Args...>(obj, fn, bytes);
}
