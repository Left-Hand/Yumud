...
static constexpr Self from(uint32_t uiqn_input){
     ...
     */
    uiq31_input = (uint32_t)uiqn_input;
...
constexpr int32_t into_bits() const{
     ...
     */
    if (exp > 0) {
        iq_n_result += uint32_t(uint64_t(UQ32_LN2) * uint32_t(((int32_t)exp << Q)) >> 32) ;
    } else {
        iq_n_result -= uint32_t(uint64_t(UQ32_LN2) * uint32_t((((uint32_t) - exp) << Q)) >> 32) ;
    }
...
constexpr math::fixed<Q, int32_t> ln32u(const math::fixed<Q, uint32_t> x) {
    constexpr uint32_t iqNMin = ((Q >= 27) ? fxmath::details::IQNLOG_MIN[Q - 27] : 1);
    const uint32_t uiqn_input = x.to_bits();
    const int32_t ret_bits = [&] -> int32_t{
        if constexpr(Q > 26) {
            if (uiqn_input <= iqNMin) {
                return INT32_MIN;
            }
        }
