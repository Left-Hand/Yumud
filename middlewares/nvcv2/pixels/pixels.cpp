...
void ostu(Image<Binary>& dst, const Image<Gray>& src){
    ...
    for(size_t i = 0; i < 256; i++){
        const int32_t current_sum = sum_map[i];
        const int32_t current_cnt = cnt_map[i];
        
        const int32_t remain_sum = total_sum - current_sum;
        const int32_t remain_cnt = total_cnt - current_cnt;
...
void calc_max_entropy(const Image<Gray>& src, const int32_t thresh){
    ...
    int32_t totalpix = size.x * size.y;
    ...
    for (size_t i = 0; i < 256; ++i){
        ...
        for (size_t j = 0; j < i; ++j){
            if (hist[j] != 0){
                ...
                HO = HO + probability*float(math::ln(uq16(1)/uq16::from(probability)));
            }
        }
        ...
        for (size_t k = i; k < 256; ++k){
            if (hist[k] != 0){
                ...
                HB = HB + probability*float(math::ln(1u/uq16::from(probability)));
            }
        }
...
int32_t get_huang_fuzzy_threshold(Histogram hist){
    ...
    iq16 entropy = 0;
    //   找到第一个和最后一个非0的色阶值
    for (First = 0; First <int32_t(hist.size()) && hist[First] == 0; First++) ;
    ...
    for (Y = 1; Y < Last + 1 - First; Y++)
    {
        ...
        // DEBUG_VALUE(mu);
        smu[Y] = -iq16(mu) * math::ln(mu) - (1 - mu) * math::ln(1 - mu);      // 公式（6）
        // DEBUG_VALUE(smu[Y]);
        // smu[Y] = math::ln(mu);
        // S
        // DEBUG_VALUE(smu[Y]);
    ...
    {
        entropy = 0;
        int32_t mu = (math::round_cast<int32_t>((iq16)W[Y] / S[Y]));             // 公式17
        for (X = First; X <= Y; X++)
            entropy = entropy + smu[ABS(X - mu)] * hist[X];
        mu = math::round_cast<int32_t>((iq16)(W[Last] - W[Y]) / (S[Last] - S[Y]));  // 公式18
