...
#include "middlewares/algebra/shapes/prelude.hpp"
...
struct [[nodiscard]] AnnularSector;
...
public:
    ...
    __fast_inline constexpr bool contains_point(
        ...
        if(p_r_squ > math::square(radius_range.stop)) return false;
...
template<typename T, typename D>
struct [[nodiscard]] is_placed_t<AnnularSector<T, D>>:std::true_type{;};
...
template<typename T, typename D>
struct [[nodiscard]] is_placed_t<RoundedAnnularSector<T, D>>:std::true_type{;};
...
template<typename T, typename D>
struct [[nodiscard]] is_placed_t<Sector<T, D>>:std::true_type{;};



template<typename T, typename D>
struct [[nodiscard]] RasterizationIterator<Sector<T, D>> {
    using Shape = Sector<T, D>;
    using Self = RasterizationIterator<Sector<T, D>>;
...
private:
    ...
    constexpr RasterizationIterator(const Shape & shape, const CtorHelper & helper):
        ...
        squ_outer_radius_(static_cast<uint32_t>(
            math::square(static_cast<iq12>(shape.radius)))),
...
template<typename T, typename D>
struct [[nodiscard]] RasterizationIterator<AnnularSector<T, D>> {
    using Shape = AnnularSector<T, D>;
    using Self = RasterizationIterator<AnnularSector<T, D>>;
...
private:
    ...
    constexpr RasterizationIterator(const Shape & shape, const CtorHelper & helper):
        ...
        squ_inner_radius_(static_cast<uint32_t>(
            math::square(static_cast<iq12>(shape.radius_range.start)))),
        squ_outer_radius_(static_cast<uint32_t>(
            math::square(static_cast<iq12>(shape.radius_range.stop)))),
        start_norm_vec_(helper.v1),
        stop_norm_vec_(helper.v2),
        is_minor_(helper.v2.is_counter_clockwise_to(helper.v1))
        {;}
...
private:
    static constexpr uint8_t s_color_from_point(const Self & self, const math::Vec2<T> offset){
        ...
        return (self.is_minor 
            ...
            : (offset.is_counter_clockwise_to(self.start_norm_vec) || 
            offset.is_clockwise_to(self.stop_norm_vec))) ? 0x80 : 0;
    } 
};
