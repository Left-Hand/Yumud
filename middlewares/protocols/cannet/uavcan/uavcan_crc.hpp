...
#if 1
// ccitt_false
[[nodiscard]] constexpr uint16_t crc16_push_byte(uint16_t crc_val, uint8_t byte) {
    uint8_t index = (crc_val >> 8) ^ byte;
    return (crc_val << 8) ^ CRC_TABLE[index];
}
#else
...
[[nodiscard]] constexpr uint16_t crc16_push_bytes(uint16_t crc_val, std::span<const uint8_t> bytes) {
    for (size_t i = 0; i < bytes.size(); i++) {
        crc_val = crc16_push_byte(crc_val, bytes[i]);
    }
    return crc_val;
}

[[nodiscard]] constexpr uint16_t crc16_push_signature(uint16_t crc_val, uint64_t data_type_signature){
    for (uint16_t shift_val = 0; shift_val <64; shift_val =(uint16_t)(shift_val +8U)){
        crc_val = crc16_push_byte(crc_val, (uint8_t) (data_type_signature >> shift_val));
    }
    return crc_val;
}
