...
//y=ax+b
void lineplofit(std::vector<ymd::Vec2<float>>& points_list, int points_num, float* a, float* b)
{
    float sum_x2 = 0.0;
    float sum_y = 0.0;
    float sum_x = 0.0;
...
void RansacPolyfitLine(std::vector<ymd::Vec2<float>> p, int iter_num, float alpha, float* a, float* b)
{
    int r_idx[RANSAC_K];
 
 
    std::vector<ymd::Vec2<float>> pick_p;
    ...
    std::vector<ymd::Vec2<float>> inline_p;
    std::vector<ymd::Vec2<float>> max_inline_p;
