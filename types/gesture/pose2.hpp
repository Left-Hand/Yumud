...
struct Pose2{
    ...
    template<typename U = T>
    constexpr Pose2(const Vec2<U> & _pos, const U _rad):
    ...
    [[nodiscard]] constexpr Pose2<T> forward_move(const T length) const {
        ...
        const auto delta = Vec2<T>{c, s} * length;
        return {
            position + delta,
            orientation
        };
    ...
    [[nodiscard]] constexpr Pose2<T> revolve_by_radius_and_rotation(
        ...
        const auto ar = Vec2<T>::from_idenity_rotation(orientation).rotated(
            rot > 0 ? T(PI/2) : T(-PI/2)
        ) * radius;
