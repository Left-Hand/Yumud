...
// 	Basis rotated(const Vector3 &p_axis, float p_phi) const;

// 	void rotate_local(const Vector3 &p_axis, float p_phi);
// 	Basis rotated_local(const Vector3 &p_axis, float p_phi) const;
...
// 	void set_axis_angle(const Vector3 &p_axis, float p_phi);
...
// 	void set_axis_angle_scale(const Vector3 &p_axis, float p_phi, const Vector3 &p_scale);
...
// 	bool is_equal_approx_ratio(const Basis &a, const Basis &b, float p_epsilon = CMP_EPSILON) const;
...
// 	inline void operator-=(const Basis &p_matrix);
// 	inline Basis operator-(const Basis &p_matrix) const;
// 	inline void operator*=(float p_val);
// 	inline Basis operator*(float p_val) const;
...
// 	Basis(const Vector3 &p_axis, float p_phi) { set_axis_angle(p_axis, p_phi); }
// 	Basis(const Vector3 &p_axis, float p_phi, const Vector3 &p_scale) { set_axis_angle_scale(p_axis, p_phi, p_scale); }
...
// inline void Basis::operator*=(float p_val) {
// 	elements[0] *= p_val;
// 	elements[1] *= p_val;
// 	elements[2] *= p_val;
// }

// inline Basis Basis::operator*(float p_val) const {
// 	Basis ret(*this);
// 	ret *= p_val;
// 	return ret;
// }
