...
public:
    ...
    constexpr Range2_t() = default;

    constexpr Range2_t(const T _from, const T _to): from(_from), to(_to) {
        *this = this -> abs();
    }

    constexpr Range2_t(const Range2_t<auto> & other): from(static_cast<T>(other.from)), to(static_cast<T>(other.to)) {
        *this = this -> abs(); 
    }

    constexpr Range2_t(const std::pair<arithmetic auto, arithmetic auto> & other): 
    from(static_cast<T>(other.first)), to(static_cast<T>(other.second)) {
        *this = this -> abs(); 
    }

    constexpr Range2_t(const std::tuple<auto, auto> & other):
    ...
    constexpr Range2_t<T> & operator=(const Range2_t<auto> & other) {
        this->from = static_cast<T>(other.from);
        this->to = static_cast<T>(other.to);
        return *this;
    }

    constexpr static Range2_t<T> from_center(const arithmetic auto & center, const arithmetic auto & length){
    ...
    constexpr Range2_t<T> abs() const{
        if(unlikely(from > to)) return Range2_t<T>(to, from);
        else return *this;
    }

    // constexpr Range2_t<T> operator + (const arithmetic auto & rvalue) const{
    ...
    __fast_inline constexpr Range2_t<T> operator / (const arithmetic auto & rvalue) const{
        return Range2_t<T>(this->from / rvalue, this->to / rvalue);
    }

    // constexpr Range2_t<T> & operator += (const arithmetic auto & rvalue) {
    //     *this = *this + rvalue;
    //     return *this;
    // }
    ...
    constexpr bool operator == (const Range2_t<auto> & other) const {
    ...
    constexpr bool intersects(const Range2_t<auto> & other) const {
        return(this->has(other.from) || other.has(this->from));
    }

    constexpr bool contains(const Range2_t<auto> & other) const {
        return (this->from <= other.from && this->to >= other.to);
    }

    constexpr bool inside(const Range2_t<auto> & other) const {
    ...
    constexpr bool has(const Range2_t<auto> & other) const{return contains(other);}

    constexpr T padding(const Range2_t<T> other) const {
        if(this->intersects(other)) return 0;
        return MIN((other.to - this->from), (other.from - this->to));
    }

    constexpr Range2_t<T> intersection(const Range2_t<auto> & other) const {
    ...
    scexpr Range2_t<T> grid(const arithmetic auto & value, const auto & grid_size){
        T ret_from;
    ...
    scexpr Range2_t<T> grid_next(const arithmetic auto & value, const arithmetic auto & grid_size, const bool right = true){
        return grid(right ? (value + grid_size) : (value-grid_size), grid_size);
    }

    scexpr Range2_t<T> grid_next_right(const arithmetic auto & value, const arithmetic auto & grid_size){
        return grid_next(value, grid_size, true);
    }

    scexpr Range2_t<T> grid_next_left(const arithmetic auto & value, const arithmetic auto & grid_size){
        return grid_next(value, grid_size, false);
    }

    constexpr Range2_t<T> gridfy(const arithmetic auto & grid_size) const {
        return Range2_t<T>(grid(from, grid_size), grid(to, grid_size));
    }

    scexpr Range2_t<T> part_in_grid(const arithmetic auto & value, const arithmetic auto & grid_size, const bool right_part = true){
    ...
    constexpr Range2_t<T> part_right_in_grid(const arithmetic auto & value, const arithmetic auto & grid_size) const{
        return part_in_grid(value, grid_size, true);
    }

    constexpr Range2_t<T> part_left_in_grid(const arithmetic auto & value, const arithmetic auto & grid_size) const{
        return part_in_grid(value, grid_size, false);
    }


    constexpr Range2_t<T> room_left(const Range2_t<auto> & content) const{
        return {this->from, MAX(content.from, this->from)};
    }

    constexpr Range2_t<T> room_right(const Range2_t<auto> & content) const{
        return {this->from, MIN(content.from, this->from)};
    }

    constexpr int rooms(const Range2_t<auto> & _content) const{
        return bool(room_left(_content)) + bool(room_right(_content));
    }

    constexpr Range2_t<T> grid_forward(const Range2_t<auto> & before, const auto & grid_size) const{
    ...
    constexpr Range2_t<T> scale(const auto & amount){
    ...
    constexpr Range2_t<T> grow(const arithmetic auto & value) const{
        const Range2_t<T> ret = Range2_t<T>(this->from - value, this->to + value);
        if (ret.is_regular()) return ret;
        else return Range2_t<T>();
    }

    constexpr Range2_t<T> merge(const Range2_t<arithmetic auto> & other) const {
        return Range2_t<T>(MIN(this->from, other.from), MAX(this->to, other.to));
    }

    constexpr Range2_t<T> shift(const arithmetic auto & value) const{
        Range2_t<T> ret = Range2_t<T>(this->from + value, this->to + value);
        return ret;
    }

    constexpr Range2_t<T> merge(const arithmetic auto & value) const{
