...
public:
    GpioVirtual(const Gpio & gpio):GpioBase(gpio.pin_index), instance(form_gpiotypedef_to_port(gpio.instance)){;}
    GpioVirtual(PortBase * _instance, const int8_t _pin_index):GpioBase(_pin_index), instance(_instance){;}
