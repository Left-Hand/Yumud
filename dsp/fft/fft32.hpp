...
constexpr math::fixed_t<Q, int32_t> dft32_bin0(
    ...
    return  math::fixed_t<Q, int32_t>::from_bits(static_cast<int32_t>(real_bits >> (N_BITS)));
}
...
static constexpr auto _make_sincos_table(){
    std::array<std::array<math::fixed_t<Q, T>, 2>, N> table;
    uq16 x = 0;
    constexpr uq16 step = uq16::from_rcp(N) * Step;
    for(size_t i = 0; i < N; i++){
        const auto [s,c] = math::sincospu(x);
        table[i] = std::to_array({
            math::fixed_t<Q, T>::from_bits(iq15(s).to_bits()), 
            math::fixed_t<Q, T>::from_bits(iq15(c).to_bits())
        });
        x += step;
    }
    return table;
...
__attribute__((optimize("O3"), hot, flatten))
constexpr std::tuple<math::fixed_t<Q, int32_t>, math::fixed_t<Q, int32_t>> _dft(
    std::span<const math::fixed_t<Q, int32_t>, (1u << N_BITS)> real_in
){
    constexpr size_t N = 1u << N_BITS;
    ...
    // 使用restrict指针
    const auto* __restrict in_ptr = real_in.data();
    const std::array<math::fixed_t<TABLE_Q, int16_t>, 2> * __restrict tab_ptr = TABLE.data();
    ...
    return {
        math::fixed_t<Q, int32_t>::from_bits(static_cast<int32_t>(real_bits >> (N_BITS + TABLE_Q))),
        math::fixed_t<Q, int32_t>::from_bits(static_cast<int32_t>(imag_bits >> (N_BITS + TABLE_Q))),
    };
}
...
struct DftCalcultor{
    ...
    static constexpr std::array<math::fixed_t<Q, D>, 2> calc_dft(
        size_t bin_num,
        std::span<const math::fixed_t<Q, D>, N> real_in
    ){ 
        int64_t real_bits = 0;
        int64_t imag_bits = 0;
        // 使用restrict指针
    ...
    static constexpr std::array<std::array<math::fixed_t<Q, D>, 2>, NUM_BINS> calc_dft(
        std::array<size_t, NUM_BINS> bins,
        std::span<const math::fixed_t<Q, D>, N> real_in
    ){ 
        std::array<std::array<math::fixed_t<Q, D>, 2>, NUM_BINS> ret;
