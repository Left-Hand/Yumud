...
void SimplePll::update(const iq_t<16> lap_pos){
    ...
    iq_t<16> pos_err = accu_pos_ - pll_pos_;
    err_int_ += (pos_err);
    pll_pos_ += (err_int_ * ki_ + pos_err * kp_);
}

iq_t<16> LapPosPll::update(const iq_t<16> lap_pos){
    const iq_t<16> pos_meas = pos_accumulator_.update(lap_pos);
    const iq_t<16> pos_err = pos_meas - pos_est_;
