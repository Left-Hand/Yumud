...
//     return fposmodp(x + iq_t<16>(PI), iq_t<16>(TAU)) - iq_t<16>(PI);
// }

scexpr __fast_inline iq_t<16> sq(const iq_t<16> x){
    return x * x;
}
...
void NonlinearObserver::update(iq_t<16> Valpha, iq_t<16> Vbeta, iq_t<16> Ialpha, iq_t<16> Ibeta){
    ...
    const iq_t<16> I_alpha_beta[2] = {Ialpha, Ibeta};
    // alpha-beta vector operations
    iq_t<16> eta_mf[2];
    for (size_t i = 0; i < 2; ++i) {
        // flux dynamics (prediction)
        iq_t<16> x_dot = -config_.phase_resistance * I_alpha_beta[i] + V_alpha_beta_last_[i];
    ...
    iq_t<16> pm_flux_sqr_mf_2 = sq(config_.pm_flux_linkage * config_.freq);
    iq_t<16> est_pm_flux_sqr_mf_2 = sq(eta_mf[0]) + sq(eta_mf[1]);
    iq_t<16> eta_factor = 0.5_r * (config_.observer_gain / pm_flux_sqr_mf_2) * (pm_flux_sqr_mf_2 - est_pm_flux_sqr_mf_2);
    ...
    for (size_t i = 0; i < 2; ++i) {
        ...
        iq_t<16> x_dot = eta_factor * eta_mf[i];
