...
namespace SMC{
    ...
    std::tuple<Point, Rangei> get_entry(const ImageReadable<Binary> & src, const Vector2i & last_seed_pos, const LR);
    ...
    namespace PileUtils{
        bool validity(const Pile & pile, const Rangei & valid_width);
        bool validity(const Piles & piles, const Rangei & valid_width);
    }
