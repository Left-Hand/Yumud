...
// @ChessBoard board 棋盘
// @return 最优的下一步棋的位置
[[nodiscard]] static constexpr math::Vec2u 
chess_forward_ai(const Role role, const ChessBoard & board){
    // 判断指定位置是否能让指定角色获胜
    auto is_winning_move = [&board](const math::Vec2u& pos, Role check_role) -> bool {
        {
            // 检查行
            size_t cnt = 0;
    ...
    // 寻找可立即获胜的位置
    auto find_winning_move = [is_winning_move, &board](const Role check_role) -> std::optional<math::Vec2u> {
        for (size_t y = 0; y < ChessBoard::WIDTH; ++y) {
            for (size_t x = 0; x < ChessBoard::WIDTH; ++x) {
                math::Vec2u pos{x, y};
    ...
    // 4. 占领四个角落
    constexpr std::array<math::Vec2u, 4> corners = {{{0,0}, {0,2}, {2,0}, {2,2}}};
    for (const auto& pos : corners) {
        if (board.at(pos) == None) return pos;
    }

    // 5. 占领边缘（非中心非角落）
    constexpr std::array<math::Vec2u, 4> edges = {{{0,1}, {1,0}, {1,2}, {2,1}}};
