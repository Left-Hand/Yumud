...
class PolarPlotter:
    ...
    def plot_point(self, r: float, theta: float, color: str = "red") -> None:
        """
        ...
        """
        # Normalize radius to fit within display
        normalized_r: float = r * 30
        x: float = self.center_x + normalized_r * math.cos(theta - 1.57)
        y: float = self.center_y + normalized_r * math.sin(theta - 1.57)
    ...
    def _process_line(self, line: str) -> None:
        ...
        try:
            ...
            if len(float_values) >= 2:
                r: float = float_values[0]
                theta: float = float_values[1]
                
                # Update plot in main thread
                self.plotter.clear_points()
                self.plotter.plot_point(r, theta)
