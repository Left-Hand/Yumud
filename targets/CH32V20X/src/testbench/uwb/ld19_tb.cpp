...
using namespace ymd;
using drivers::LD19_Prelude;
using drivers::LD19_StreamParser;
...
void ld19_main(){
    ...
    using LD19Event = LD19_Prelude::Event;


    auto ld19_ev_handler = [&](const LD19Event & ev){ 
        if(ev.is<LD19Event::DataReady>()){
            const auto & packet = ev.unwrap_as<LD19Event::DataReady>().frame;
    ...
    auto ld19_parser = drivers::LD19_StreamParser(ld19_ev_handler);
    ld19_uart.init({
        LD19_Prelude::DEFAULT_UART_BAUD
    });
