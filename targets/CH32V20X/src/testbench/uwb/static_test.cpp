...
constexpr auto test_no_intersection = []->bool  {
    Circle2<double> circle1{Vec2<double>(0, 0), 1.0};
    Circle2<double> circle2{Vec2<double>(5, 0), 1.0};
...
constexpr auto test_contained = []->bool  {
    Circle2<double> circle1{Vec2<double>(0, 0), 5.0};
    Circle2<double> circle2{Vec2<double>(1, 0), 1.0};
    auto result = compute_intersection_points(circle1, circle2);
...
constexpr auto test_tangent = []->bool  {
    Circle2<double> circle1{Vec2<double>(0, 0), 2.0};
    Circle2<double> circle2{Vec2<double>(4, 0), 2.0};
    auto result = compute_intersection_points(circle1, circle2);
...
constexpr auto test_exact_intersection_int = []->bool  {
    constexpr Circle2<float> circle1{Vec2<float>(0, 0), 5};  // 圆心在原点，半径5
    constexpr Circle2<float> circle2{Vec2<float>(6, 0), 5};  // 圆心在(6,0)，半径5
    ...
    static_assert(is_equal_approx(result[0].x, 3.0f), "浮点数测试失败");
    static_assert(is_equal_approx(result[0].y, 4.0f), "浮点数测试失败");
    
    // 检查第二个交点 (3,-4)  
    // if (result[1].x != 3 || result[1].y != -4) return false;
    static_assert(is_equal_approx(result[1].x, 3.0f), "浮点数测试失败");
    static_assert(is_equal_approx(result[1].y, -4.0f), "浮点数测试失败");
...
constexpr auto test_negative_radius = []->bool  {
    Circle2<double> circle1{Vec2<double>(0, 0), -1.0}; // 负半径
    Circle2<double> circle2{Vec2<double>(1, 0), 1.0};
    auto result = compute_intersection_points(circle1, circle2);
...
constexpr auto compile_time_test = []{
    Circle2<double> circle1{Vec2<double>(0, 0), 3.0};
    Circle2<double> circle2{Vec2<double>(4, 0), 2.0};
