...
real_t demo(uint milliseconds, uint microseconds = 0){
    ...
    const uint32_t turnCnt = milliseconds % 2667;
    const uint32_t turns = milliseconds / 2667;
    
    scexpr real_t vel_points[7] = {
        real_t(20)/360, 
        real_t(20)/360, 
        real_t(62.4)/360, 
        real_t(62.4)/360, 
    ...
    const auto delta_t = solver.tb - solver.ta;
    const real_t delta_p = solver.pb - solver.pa;

    const real_t _t = ((real_t)(turnCnt  - solver.ta) / delta_t);
    const real_t temp = (real_t)delta_t / 1000 / delta_p; 

    const auto yt = [&]{
        if((i == 0) || (i == 2) || (i == 4))
            return CubicInterpolation::forward(
                Vector2<real_t>{real_t(0.4f), real_t(0.4f) * solver.va * temp}, 
                Vector2<real_t>(real_t(0.6f), real_t(1.0f) - real_t(0.4f)  * solver.vb * temp), _t);
        else
            return _t;
    }();

    return real_t(turns) + solver.pa + delta_p * yt;
}
