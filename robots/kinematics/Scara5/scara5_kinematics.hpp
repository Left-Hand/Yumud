...
public:
    ...
    constexpr Vec2<T> forward(const Angular<T> left,const Angular<T> right){
        const auto l_pos = Vec2<T>(-cfg_.should_length / 2, 0);
        const auto r_pos = Vec2<T>(cfg_.should_length / 2, 0);

        const Vec2<T> l_elbow_pos = l_pos + Vec2<T>(cfg_.upperarm_length, 0).rotated(left);
        const Vec2<T> r_elbow_pos = r_pos + Vec2<T>(cfg_.upperarm_length, 0).rotated(right);
...
constexpr std::tuple<Angular<T>, Angular<T>> inverse(const Vec2<T> & pos){
        const auto l_pos = Vec2<T>(-cfg_.should_length / 2, 0);
        const auto r_pos = Vec2<T>(cfg_.should_length / 2, 0);
