...
// public:
    ...
    constexpr auto d2_forward_leg(const T hip_rad, const T knee_rad) const {
        auto knee_pos = Vector2<T>(config.thigh_length_meter, 0).rotated(-hip_rad);
        auto feet_pos = knee_pos + Vector2<T>(config.shin_length_mster, 0).rotated(T(PI)-hip_rad - knee_rad);
...
public:
    ...
    constexpr auto d3_forward_leg(const T hip_rad, const T knee_rad, bool is_right) const {
        ...
        auto transform_d2_to_d3 = [](const Vector2<T> & d2_pos) -> Vector3<T> {return Vector3<T>(0, d2_pos.y, -d2_pos.x);};
        auto knee_pos = hip_pos + transform_d2_to_d3(d2_knee_pos);
        auto feet_pos = hip_pos + transform_d2_to_d3(d2_feet_pos);
        return std::make_tuple(knee_pos, feet_pos);
    }

    constexpr auto d3_inverse_leg(const Vector3<T> feet_pos, bool is_right) const {
        auto hip_pos = Vector3<T>(is_right ? (config.pelvis_length_meter / 2) : (- config.pelvis_length_meter / 2), 0, 0);
        auto delta_pos = feet_pos - hip_pos;
        auto transform_d3_to_d2 = [](const Vector3<T> & d3_pos) -> Vector2<T> {return Vector2<T>(-d3_pos.x, d3_pos.y);};
    ...
    constexpr auto transform_ground_to_pelvis(const Vector3<T> & left_feet_pos, const Vector3<T> & right_feet_pos, const T pitch_rad) const {
