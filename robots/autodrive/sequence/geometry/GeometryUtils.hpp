...
std::tuple<std::optional<ymd::Vector2<T>>, std::optional<ymd::Vector2<T>>> calculate_circle_center(const ymd::Vector2<T> & p1, const ymd::Vector2<T> & p2, const T radius){
    const ymd::Vector2<T> mid_point = p1.center(p2);

    const ymd::Vector2<T> vec_p1_p2 = p2 - p1;
    ...
    // 计算法线向量
    const ymd::Vector2<T> normal_vector = ymd::Vector2<T>(-vec_p1_p2.y, vec_p1_p2.x).normalized();

    // 计算圆心到中点的距离
    const T d = sqrt(radius * radius - (distance_squ_div4));

    // 计算两个可能的圆心
    const ymd::Vector2<T> center1 = mid_point + normal_vector * d;
    const ymd::Vector2<T> center2 = mid_point - normal_vector * d;
...
template<arithmetic T>
std::optional<ymd::Vector2<T>> calculate_fillet_center(const ymd::Line2_t<T> & a, const ymd::Line2_t<T> & b, const T & radius){
    return std::nullopt;
}
...
ymd::Vector2<T> get_square_rounded_position(const ymd::Rect2<T> & rect, const uint8_t index){
    index = index - 1;
    ...
    return ymd::Vector2<T>(pos_begin.x + (pos_end.x - pos_begin.x) * x_i, pos_begin.y + (pos_end.y - pos_begin.y) * y_i);
}

template<arithmetic T>
ymd::Vector2<T> get_square_rounded_position(const ymd::Rect2<T> & rect, const T & rad){
    ...
    return pos_center + ymd::Vector2<T>{-distance, 0}.rotated(rad);
}
