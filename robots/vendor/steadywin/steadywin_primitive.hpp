...
namespace ymd::robots::steadywin{
...
struct [[nodiscard]] MotorFlags final{ 
    ...
    // bit0：电机异常位（odrv0.axis0.motor.error 是否为 0）
    // bit1：编码器异常位（odrv0.axis0.encoder.error是否为 0）
    // bit2：控制异常位（odrv0.axis0.controller.error是否为 0）
    // bit3：系统异常位（odrv0.error 是否为 0）
    // bit7：odrv0.axis0.controller.trajectory_done，即位置曲线是否执行完毕
