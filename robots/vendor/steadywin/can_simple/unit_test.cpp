...
static_assert(sizeof(EncoderErrorFlags) == 4, "EncoderErrorFlags should be 4 bytes");
static_assert(sizeof(MotorErrorFlags) == 4, "MotorErrorFlags should be 4 bytes");
...
// 测试位域转换
static_assert(AxisErrorFlags::from_bits(0).invalid_state == 0, "Invalid state bit should be 0");
static_assert(AxisErrorFlags::from_bits(1).invalid_state == 1, "Invalid state bit should be 1");
static_assert(EncoderErrorFlags::from_bits(1).unstable_gain == 1, "Unstable gain bit should be 1");
static_assert(MotorErrorFlags::from_bits(1).phase_resistance_out_of_range == 1, "Phase resistance bit should be 1");

}
[[maybe_unused]] void test2(){

// 测试 FrameId 编码和解码
static constexpr auto test_axis_id = AxisId::from_bits(0b101010);
static constexpr Command test_command(Command::SetInputCurrent);
static constexpr FrameId test_frame_id{test_axis_id, test_command};
...
static_assert(decoded_frame_id.axis_id.to_bits() == test_axis_id.to_bits(), "Axis ID should match after round-trip");
...
static_assert(decoded_test.axis_id.to_bits() == 0b101010, "Axis ID decoding test");
static_assert(decoded_test.command.kind() == Command::GetEncoderError, "Command decoding test");

}
[[maybe_unused]] void test3(){
// 测试 FrameId 编码和解码
static constexpr AxisId test_axis_id = AxisId::from_bits(0b101010);
static constexpr Command test_command(Command::SetInputCurrent);
...
// 测试命令枚举值
static_assert(static_cast<uint8_t>(Command::Undefined) == 0, "Undefined command should be 0");
static_assert(static_cast<uint8_t>(Command::OdriveHeartbeat) == 1, "OdriveHeartbeat command should be 1");
static_assert(static_cast<uint8_t>(Command::OdriveEstop) == 2, "OdriveEstop command should be 2");
static_assert(static_cast<uint8_t>(Command::GetMotorError) == 3, "GetMotorError command should be 3");

}
[[maybe_unused]] void test5(){
// 测试 Command 结构体
static constexpr Command test_cmd(Command::SetAxisNodeId);
static_assert(test_cmd.kind() == Command::SetAxisNodeId, "Command kind should match");
static_assert(Command::from_bits(static_cast<uint8_t>(Command::ResetOdrive)).kind() == Command::ResetOdrive, "Command from_bits should work");
...
// 测试 AxisId 结构体
static constexpr auto test_axis_id = AxisId::from_bits(42);
static_assert(test_axis_id.to_bits() == 42, "AxisId bits should match");
static_assert(AxisId::from_bits(123).to_bits() == 123, "AxisId from_bits should work");
...
static constexpr FrameId heartbeat_frame_id{test_axis_id, Command(Command::OdriveHeartbeat)};
...
static_assert(heartbeat_result.unwrap().identifier().to_stdid().to_u11() == ((1 << 5) | static_cast<uint8_t>(Command::OdriveHeartbeat)), "Heartbeat frame ID should be correct");

// 测试带一个参数的消息序列化
static constexpr FrameId set_state_frame_id{test_axis_id, Command(Command::SetAxisRequestedState)};
...
// 测试带两个参数的消息序列化
static constexpr FrameId set_controller_frame{test_axis_id, Command(Command::SetControllerModes)};
...
[[maybe_unused]] void test_de(){
    // 测试编码和解码一致性
static constexpr AxisId axis_id = AxisId::from_bits(5);
static constexpr Command command(Command::GetEncoderEstimates);
...
static_assert(deserialize_result.unwrap().axis_id.to_bits() == 5, "Axis ID should match");
...
[[maybe_unused]] void test_cmd(){
    // 测试紧急停止命令
static constexpr auto estop_result = FrameSerializer::emergency_stop(AxisId::from_bits(3));
static_assert(estop_result.is_ok(), "Emergency stop creation should succeed");
static_assert(estop_result.unwrap().identifier().to_stdid().to_u11() == ((3 << 5) | static_cast<uint8_t>(Command::OdriveEstop)), "Estop frame ID should be correct");

// 测试重启命令
static constexpr auto reboot_result = FrameSerializer::reboot(AxisId::from_bits(2));
static_assert(reboot_result.is_ok(), "Reboot creation should succeed");
static_assert(reboot_result.unwrap().identifier().to_stdid().to_u11() == ((2 << 5) | static_cast<uint8_t>(Command::ResetOdrive)), "Reboot frame ID should be correct");

// 测试清除错误命令
static constexpr auto clear_errors_result = FrameSerializer::clear_errors(AxisId::from_bits(4));
static_assert(clear_errors_result.is_ok(), "Clear errors creation should succeed");
static_assert(clear_errors_result.unwrap().identifier().to_stdid().to_u11() == ((4 << 5) | static_cast<uint8_t>(Command::ClearErrors)), "Clear errors frame ID should be correct");

// 测试设置轴ID命令
static constexpr auto set_axis_id_result = FrameSerializer::set_axis_node_id(AxisId::from_bits(1), AxisId::from_bits(10));
