...
static __inline Vector2_t<real_t> signed_sqrt(const Vector2_t<real_t> vec){
...
Vector2_t<real_t> PositionCtrl::update(const Vector2_t<real_t> & targ_pos, const Vector2_t<real_t> & pos, const Vector2_t<real_t> & spd){
    auto pos_err = targ_pos - pos;
    auto spd_err = Vector2_t<real_t>(0,0) - spd;
    return {
