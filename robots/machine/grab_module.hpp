...
public:
    struct Config{
        std::array<Vector2q<16>, 3> tray_xy;
        real_t tray_z;
        real_t free_z;
        real_t catch_z;
        real_t z_bias;
        
        std::array<Vector2q<16>, 3> catch_xy;
        Vector3q<16> inspect_xyz;
        Vector3q<16> idle_xyz;

        Vector3q<16> home_xyz;
...
// protected:
    ...
    void rapid(const Vector3q<16> & pos);//改变所有坐标
    Vector3q<16> getPos();
...
public:
    ...
    void init();
    void move(const Vector3q<16> & pos);
    void move_xy(const Vector2q<16> & pos);
