...
protected:
    ...
    void init(const Vec3<real_t> & from){
        from_ = (from);
...
public:
    ACTION_NAME(move)
    MoveAction(Inst & inst, const Vec3<real_t> & to):
...
public:
    ...
    MoveZAction(Inst & inst, const real_t to):
        GrabAction(UINT32_MAX, [this](){
            ...
            auto dest_pos = Vec3<real_t>(from_.xy(), from_.z + solver_->forward(time));
...
public:
    ACTION_NAME(movexy)
    MoveXYAction(Inst & inst, const Vec2<iq16> to):
        GrabAction(UINT32_MAX, [this](){
            if(this->first()){//将计算延迟到实际开始运行而不是构造时 确保起始位置正确
                init(inst_.getPos());
            }
            ...
            auto dest_pos = Vec3<real_t>(from_.xy() + norm_ * solver_->forward(time), from_.z);
...
public:
    ACTION_NAME(rapid)
    RapidAction(Inst & inst, const Vec3<real_t> & to):
        GrabAction(UINT32_MAX, [this](){
            if(first()){
                inst_.rapid(to_);
            }
