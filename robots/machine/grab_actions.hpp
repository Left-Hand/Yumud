...
protected:
    ...
    Vector3_t<real_t> to_;
    Vector3_t<real_t> from_;
    real_t dist_;
    Vector3_t<real_t> norm_;
    std::optional<TrapezoidSolver> solver_ = std::nullopt;
    real_t dur_;

    void init(const Vector3_t<real_t> & from){
        from_ = (from);
...
public:
    ACTION_NAME(move)
    MoveAction(Inst & inst, const Vector3_t<real_t> & to):
...
public:
    ...
    MoveZAction(Inst & inst, const real_t to):
        GrabAction(UINT32_MAX, [this](){
            ...
            auto dest_pos = Vector3_t<real_t>(from_.xy(), from_.z + solver_->forward(time));
...
public:
    ...
    MoveXYAction(Inst & inst, const Vector2 to):
        GrabAction(UINT32_MAX, [this](){
            ...
            auto dest_pos = Vector3_t<real_t>(from_.xy() + norm_ * solver_->forward(time), from_.z);
...
public:
    ACTION_NAME(rapid)
    RapidAction(Inst & inst, const Vector3_t<real_t> & to):
        GrabAction(UINT32_MAX, [this](){
            if(first()){
                inst_.rapid(to_);
            }
