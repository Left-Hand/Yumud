...
template<arithmetic T>
__fast_inline constexpr
Vector3<Norm_t<T>> build_norm_vec3_from_vec3(const Vector3<T> & vec){
...
template<arithmetic T>
__fast_inline constexpr
Vector3<Norm_t<T>> build_norm_vec3_from_cross_of_vec3(Vector3<Norm_t<T>> const & v1, Vector3<Norm_t<T>> const & v2){
	return Vector3<Norm_t<T>>(
...
void Mahony::update_v2(const V3 & gyr,const V3 & acc) {
...
void Mahony::myupdate(const V3 & gyr,const V3 & acc) {
...
void Mahony::myupdate_v2(const V3 & gyr,const V3 & acc) {
	...
	// const auto gyr_hat_d = tau * tau * V3{euler.x, euler.y, euler.z} + 2 * tau * (kd_ * (gyr - gyr_hat_));
	const auto gyr_hat_d = 2 * tau * (gyr - gyr_hat_);
	// const auto gyr_hat_d = tau * tau * V3{euler.x, euler.y, euler.z};
	//  + 2 * tau * (gyr - gyr_hat_);
...
void Mahony::update(const V3 & gyr,const V3 & acc) {
