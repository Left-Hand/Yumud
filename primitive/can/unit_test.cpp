...
static_assert(std::is_standard_layout_v<CanClassicMsg>);
static_assert(std::is_trivially_copyable_v<CanClassicMsg>);
static_assert(sizeof(CanClassicMsg) == 16, "CanClassicMsg should be 16 bytes due to alignment");
...
static_assert([]{
    constexpr auto msg = CanClassicMsg::from_remote(CanStdId::from_bits(0x123U));
    return msg.is_remote() && msg.id_u32() == 0x123U;
}());

static_assert([]{
    constexpr auto msg = CanClassicMsg::from_empty(CanStdId::from_bits(0x456U));
...
static_assert([]{
    constexpr auto remote_msg = CanClassicMsg::from_remote(CanStdId::from_bits(0x123U));
    constexpr auto data_msg = CanClassicMsg::from_empty(CanStdId::from_bits(0x456U));
    return remote_msg.is_remote() && !data_msg.is_remote();
}());

// Test standard vs extended ID
static_assert([]{
    constexpr auto std_msg = CanClassicMsg::from_empty(CanStdId::from_bits(0x123U));
