...
enum class [[nodiscard]] KeyCode_Kind:uint8_t{
    Backquote,
...
public:
    ...
    template<size_t I>
    static constexpr KeyCode from_digit() {
        return from_digit(I).unwrap();
    }

    static constexpr Option<KeyCode> from_alpha(const char ch){
        switch(ch){
            case 'a' ... 'z':
                return Some(KeyCode(std::bit_cast<Kind>(uint8_t(
                    std::bit_cast<uint8_t>(Kind::KeyA)
                    + (ch - 'a')
                ))));
    ...
    static constexpr Option<KeyCode> from_char(const char ch) {
        switch(ch){
            case '0' ... '9':
                return from_digit(ch - '0');
            case 'a' ... 'z':
                return from_alpha(ch);
            case 'A' ... 'Z':
                return from_alpha(ch);
            case ' ':
                return Some(KeyCode(Kind::Space));
            case '+':
                return Some(KeyCode(Kind::NumpadAdd));
            case '-':
                return Some(KeyCode(Kind::NumpadSubtract));
            case '*':
    ...
    template<char C>
    static constexpr KeyCode from_char() {
