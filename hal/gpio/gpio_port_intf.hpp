...
class GpioPortIntf{
public:
    virtual void write_nth(const size_t index, const BoolLevel data);
    virtual BoolLevel read_nth(const size_t index);
    virtual void set_nth(const size_t index);
    virtual void clr_nth(const size_t index);
...
__fast_inline void GpioPortIntf::set_nth(const size_t index){
    if(index < 0) return;
    set_by_mask(PinMask::from_nth(index));
}

__fast_inline void GpioPortIntf::clr_nth(const size_t index){
    if(index < 0) return;
    clr_by_mask(PinMask::from_nth(index));
}
...
__fast_inline BoolLevel GpioPortIntf::read_nth(const size_t index){
