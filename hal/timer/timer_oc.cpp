...
//     auto gpio = timer::details::get_pin(SPL_INST(inst_), remap, sel_);
//     if(en == EN) gpio.afpp();
// }


void TimerOutBase::set_valid_level(const BoolLevel level){
    if(level == LOW) SPL_INST(inst_)->CCER |= (1 << (std::bit_cast<uint8_t>(sel_) * 2 + 1));
    else SPL_INST(inst_)->CCER &= (~(1 << ((std::bit_cast<uint8_t>(sel_)) * 2 + 1)));
}

TimerOC::TimerOC(void * inst, const ChannelSelection sel):
    TimerOutBase(inst, sel), 
        cvr_(from_channel_to_cvr(inst, sel)), 
        arr_(SPL_INST(inst_)->ATRLR){;}
...
void TimerOC::set_oc_mode(const TimerOC::Mode mode){
    ...
    switch(sel_.kind()){
        ...
        case ChannelSelection::CH1:{
            uint16_t tmpccmrx = SPL_INST(inst_)->CHCTLR1;
            ...
            SPL_INST(inst_)->CHCTLR1 = tmpccmrx;
            break;
        }
        ...
        case ChannelSelection::CH3:{
            uint16_t tmpccmrx = SPL_INST(inst_)->CHCTLR2;
            const uint16_t m_code = TIM_OC3M;
            const uint16_t s_code = TIM_CC3S;
            tmpccmrx &= uint16_t(~(uint16_t(m_code)));
            tmpccmrx &= uint16_t(~(uint16_t(s_code)));
            tmpccmrx |= uint16_t(bits);
            SPL_INST(inst_)->CHCTLR2 = tmpccmrx;
            break;
        }
...
void TimerOutBase::enable_output(const Enable en){
    if(en == EN) SPL_INST(inst_)->CCER |= (1 << (std::bit_cast<uint8_t>(sel_) * 2));
    else SPL_INST(inst_)->CCER &= (~(1 << ((std::bit_cast<uint8_t>(sel_)) * 2)));
}
...
void TimerOC::enable_cvr_sync(const Enable en){
    ...
    switch(sel_.kind()){
        case ChannelSelection::CH1:
            TIM_OC1PreloadConfig(SPL_INST(inst_), e_code);
            break;
        case ChannelSelection::CH2:
            TIM_OC2PreloadConfig(SPL_INST(inst_), e_code);
            break;
        case ChannelSelection::CH3:
            TIM_OC3PreloadConfig(SPL_INST(inst_), e_code);
            break;
        case ChannelSelection::CH4:
            TIM_OC4PreloadConfig(SPL_INST(inst_), e_code);
            break;
        default:
            __builtin_trap();
            break;
...
// Gpio TimerOC::io(){
//     return timer::details::get_pin(SPL_INST(inst_), remap, sel_);
// }
