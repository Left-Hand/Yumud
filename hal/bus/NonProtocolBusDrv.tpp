...
BusError NonProtocolBusDrv<BusType>::write_burst(const is_stdlayout auto & data, const size_t len, Continuous cont) {
    if (BusError::OK == bus_.begin(index_) ) {
...
template <typename U>
requires std::is_standard_layout_v<U> and is_writable_bus<BusType>
BusError NonProtocolBusDrv<BusType>::write_burst(const is_stdlayout auto * data_ptr, const size_t len, Continuous cont) {
    if (BusError::OK == bus_.begin(index_) ) {
        if (sizeof(U) != 1) this->set_data_width(sizeof(U) * 8);
...
BusError NonProtocolBusDrv<BusType>::read_burst(T * data_ptr, const size_t len, const Continuous cont) {
    if (BusError::OK == bus_.begin(index_) ) {
