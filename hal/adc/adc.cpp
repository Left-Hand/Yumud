...
void AdcPrimary::init(
    ...
    { 
        ...
        for(auto config : regular_list){
            i++;
            ADC_RegularChannelConfig(instance_,(uint8_t)config.channel,i,(uint8_t)config.cycles);
...

    {
        ...
        for(auto config : injected_list){
            ...
            ADC_InjectedChannelConfig(instance_,(uint8_t)config.channel,i,(uint8_t)config.cycles);
            ADC_SetInjectedOffset(instance_, ADC_InjectedChannel_1 + (ADC_InjectedChannel_2 - ADC_InjectedChannel_1) * (i-1),MAX(cali_data, 0)); // offset can`t be negative
            __adc_internal::install_pin(config.channel);
