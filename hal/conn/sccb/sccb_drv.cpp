...
hal::HalResult SccbDrv::write_reg(const uint8_t reg_addr, const uint16_t reg_val){
    ...
    //#region 写入地址字节和第一个字节
    if(const auto res = i2c_.write_byte(reg_addr);
        res.is_err()) return res;
    if(const auto res = i2c_.write_byte(static_cast<uint8_t>(reg_val >> 8));
        res.is_err()) return res;
    //#endregion
    ...
    if(const auto res = i2c_.write_byte(0xF0); 
        res.is_err()) return res;
...
hal::HalResult SccbDrv::read_reg(const uint8_t reg_addr, uint16_t & reg_val){
    ...
    if(const auto res = i2c_.borrow(slave_addr_.with_read());
        res.is_err()) return res;
    if(const auto res = i2c_.read_byte(data_h, NACK);
        res.is_err()) return res;
    ...
    if(const auto res = i2c_.borrow(slave_addr_.with_read());
        res.is_err()) return res;
    if(const auto res = i2c_.read_byte(data_l, NACK);
        res.is_err()) return res;
