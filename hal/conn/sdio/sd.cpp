...
void SD::enable_rcc(const Enable en){
...
SD_Error SD::read_burstBlocks( uint8_t *buf, uint64_t addr, uint16_t blksize, uint32_t nblks )
{
    SD_Error err = SD_OK;
    uint8_t power;
    uint32_t count = 0;
...
SD_Error SD::write_burstBlocks(const uint8_t *buf, uint64_t addr, uint16_t blksize, uint32_t nblks )
{
    SD_Error err = SD_OK;
    uint8_t  power = 0, cardstate = 0;
    uint32_t timeout = 0, bytestransferred = 0;
...
SD_Error SD::ReadDisk( uint8_t *buf, uint32_t sector, uint8_t cnt )
{
    ...
    else
    {
        ...
        {
            sta = read_burstBlocks( buf, lsector, 512, cnt );
        }
    }
    return sta;
}
...
SD_Error SD::WriteDisk(const uint8_t *buf, uint32_t sector, uint8_t cnt ){
    ...
    if((uint32_t)buf % 4 != 0 ){
        ...
        if(cnt == 1){
            ...
            sta = write_burstBlocks(buf, lsector, 512, cnt);
        }
    }
    return sta;
}
