...
private:
    constexpr Line2(const T _d, const Angular<T> angle): 
...
public:
    ...
    [[nodiscard]] static constexpr Line2 from_dist_and_angle(const T _d, const Angular<T> angle){
    ...
    Line2<T> abs() const {
        ...
        if(d < 0){
            ...
            return {-d, Angular<T>::from_turns(m)};
        }else{
            return {d, Angular<T>::from_turns(m)};
        }
    }
    ...
    bool is_equal_approx(const Line2 & other, const T epsilon) const{
        ...
        and math::is_equal_approx(regular.orientation.to_turns(), other_regular.orientation.to_turns(), epsilon);
    }
    ...
    bool is_parallel_with(const Line2 & other, const T epsilon) const{
        ...
        math::is_equal_approx((2 * (other_regular.orientation - regular.orientation)).to_turns(), T(0), epsilon);
    }
