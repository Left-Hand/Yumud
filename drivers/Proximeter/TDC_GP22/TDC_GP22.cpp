...
IResult<> Self::validate(){
    // return transport_.transrecive_u8(0xB5); // 显示写寄存器1中的高8位, 用来测试通信
    TODO();
    return Ok();
}
...
IResult<> Self::reconf() {
    if(const auto res = transport_.write_u32(0x80009620); // 测量范围1，4M晶振二分频，4M晶振上电后一直起振，自动校准，校准陶瓷晶振时间为8个32K周期，上升沿敏感
        res.is_err()) return res;
    if(const auto res = transport_.write_u32(0x81014100); // STOP1-START
        res.is_err()) return res;
    if(const auto res = transport_.write_u32(0x82E00000); // 开启所有中断源，单边沿敏感
        res.is_err()) return res;
    if(const auto res = transport_.write_u32(0x83180000); // 几乎全为默认值
        res.is_err()) return res;
    if(const auto res = transport_.write_u32(0x84200000); // 全为默认值
        res.is_err()) return res;
    if(const auto res = transport_.write_u32(0x85080000); // 关闭噪声单元
        res.is_err()) return res;
    if(const auto res = transport_.write_u32(0x86010000); // 测量精度从90ps加倍到45ps
        res.is_err()) return res;
...
IResult<> Self::start_measurement() {
    const auto res = transport_.transrecive_u8(0x70); // Init
