...
void AD9959::init(){
    core_clock = 0;
    last_channels = ChannelSelection::All;
...
void AD9959::reset(CFR_Bits cfr){
    ...
    last_channels = ChannelSelection::Nil;        // Ensure channels get set, not optimised out
    set_channels(ChannelSelection::All);
    write(Register::CFR, uint8_t(cfr));

    set_channels(ChannelSelection::Nil);           // Disable all channels, set 3-wire MSB mode:
...
void AD9959::set_frequency(ChannelSelection chan, uint32_t freq){
    set_delta(chan, frequency_delta(freq));
}

void AD9959::set_delta(ChannelSelection chan, uint32_t delta){
    set_channels(chan);
    write(Register::CFTW, delta);
}

void AD9959::set_amplitude(ChannelSelection chan, uint16_t amplitude){        // Maximum amplitude value is 1024
...
void AD9959::set_phase(ChannelSelection chan, uint16_t phase){                // Maximum phase value is 16383
    set_channels(chan);
...
void AD9959::sweep_frequency(ChannelSelection chan, uint32_t freq, bool follow){       // Target frequency
    sweep_delta(chan, frequency_delta(freq), follow);
}

void AD9959::sweep_delta(ChannelSelection chan, uint32_t delta, bool follow){
    set_channels(chan);
...
void AD9959::sweep_amplitude(ChannelSelection chan, uint16_t amplitude, bool follow){  // Target amplitude (half)
    set_channels(chan);
...
void AD9959::sweep_phase(ChannelSelection chan, uint16_t phase, bool follow){          // Target phase (180 degrees)
    set_channels(chan);
...
void AD9959::sweep_rates(ChannelSelection chan, uint32_t increment, uint8_t up_rate, uint32_t decrement, uint8_t down_rate){
...
void AD9959::set_channels(ChannelSelection chan){
