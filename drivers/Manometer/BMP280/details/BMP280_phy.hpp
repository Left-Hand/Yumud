...
public:
    ...
    IResult<> write_reg(const RegAddr reg_addr, const uint16_t reg_val){
        if(const auto res = i2c_drv_.write_reg(uint8_t(reg_addr), reg_val, LSB);
            res.is_err()) return Err(res.unwrap_err());
        return Ok();
    }

    IResult<> read_reg(const RegAddr reg_addr, uint16_t & reg_val){
        if(const auto res = i2c_drv_.read_reg(uint8_t(reg_addr), reg_val, LSB);
            res.is_err()) return Err(res.unwrap_err());
        return Ok();
    }
    ...
    IResult<> read_burst(const RegAddr reg_addr, const std::span<int16_t> pbuf){
        if(const auto res = i2c_drv_.read_burst(uint8_t(reg_addr), pbuf, MSB);
            res.is_err()) return Err(res.unwrap_err());
        return Ok();
    }
