...
IResult<real_t> SC8815::get_bus_volt(){
    ...
    if(const auto res = read_reg(reg); 
        res.is_err()) return Err(res.unwrap_err());
    return Ok(inv_b10(reg.as_val(), 1) * ratio / 1000);
}
...
IResult<real_t> SC8815::get_bus_curr(){
    ...
    return Ok(0.05_r * ratio * inv_b10(uint16_t(reg.as_val()), 1) / 
...
IResult<real_t> SC8815::get_bat_volt(){
    ...
    if(const auto res = read_reg(reg);
        res.is_err()) return Err(res.unwrap_err());
    return Ok(inv_b10(reg.as_val(), 1) * ratio * 2 / 1000);
}
...
IResult<real_t> SC8815::get_bat_curr(){
    ...
    if(const auto res = read_reg(reg);
        res.is_err()) return Err(res.unwrap_err());
    return Ok((0.05_r * ratio * inv_b10(reg.as_val(), 1) / 
        (3 * bus_shunt_res_mohms_)));
}
...
IResult<real_t> SC8815::get_adin_volt(){
    ...
    return Ok(real_t(inv_b10(reg.as_val(), 1)) / 1000);
}
