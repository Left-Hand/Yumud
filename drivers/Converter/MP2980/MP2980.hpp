...
public:
    ...
    IResult<> set_feed_back_vref(const uq10 vref);
    IResult<> set_feed_back_vref_mv(const uint32_t vref_mv);
    IResult<> enable_power_switching(const Enable en);
    IResult<> enable_vref_change_func(const Enable en);
    IResult<> set_png_state(const bool state);
    IResult<> enable_dither(const Enable en);
    IResult<> set_vref_slew_rate(const VrefSlewRate slewrate);
    IResult<> set_ovp_mode(const OvpMode mode);
