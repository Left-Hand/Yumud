...
void AT24CXX::writePool(const size_t addr, const uint8_t * data, const size_t len){
    ...
    if (is_small_chip()){
        i2c_drv_.writeMulti(uint8_t(addr), std::span(data, len));
    }else{
        i2c_drv_.writeMulti((uint16_t)addr, std::span(data, len));
...
void AT24CXX::readPool(const size_t addr, uint8_t * data, const size_t len){
    ...
    if (is_small_chip()){
        ...
        i2c_drv_.readMulti((uint16_t)addr, std::span(data, len));
    }
}
