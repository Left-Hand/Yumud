...
protected:
    ...
    BusError write_byte(const uint8_t data, const Continuous cont = DISC){
        return spi_drv_.write_single<uint8_t>(data, cont);
    }

    BusError write_byte(const Command cmd, const Continuous cont = DISC){
        return write_byte(uint8_t(cmd), cont);
    }

    BusError write_bytes(const void * data, const size_t len){
        return spi_drv_.write_burst<uint8_t>(reinterpret_cast<const uint8_t *>(data), len);
    }

    BusError read_byte(uint8_t & data, const Continuous cont = DISC){
        return spi_drv_.read_single<uint8_t>(data, cont);
    }

    BusError read_bytes(void * data, const size_t len){
        // DEBUGGER.print("nr");
