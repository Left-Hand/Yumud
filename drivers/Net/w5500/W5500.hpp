...
void W5500::getGatewayIp(uint8_t *_addr) {
  readGAR(_addr);
}

void W5500::setGatewayIp(uint8_t *_addr) {
  writeGAR(_addr);
}

void W5500::getSubnetMask(uint8_t *_addr) {
  readSUBR(_addr);
}

void W5500::setSubnetMask(uint8_t *_addr) {
  writeSUBR(_addr);
}

void W5500::getMACAddress(uint8_t *_addr) {
  readSHAR(_addr);
}

void W5500::setMACAddress(uint8_t *_addr) {
  writeSHAR(_addr);
}

void W5500::getIPAddress(uint8_t *_addr) {
  readSIPR(_addr);
}

void W5500::setIPAddress(uint8_t *_addr) {
  writeSIPR(_addr);
}

void W5500::setRetransmissionTime(uint16_t _timeout) {
  writeRTR(_timeout);
}

void W5500::setRetransmissionCount(uint8_t _retry) {
  writeRCR(_retry);
}

void W5500::setPHYCFGR(uint8_t _val) {
  writePHYCFGR(_val);
}
