...
*/
static constexpr void WDT_QA_Calculate_Question(
...
static constexpr TCAN1145_Prelude::wdt_bits WDT_QA_Generate_Question_Bit_Array(
    uint8_t config
){
    TCAN1145_Prelude::wdt_bits bitArray;
    switch (config) {
        default:
 ...
 * @param answer[] is an array of at least 4 bytes used to store the calculated answers
 */
static constexpr void WDT_QA_Calculate_Answer(
    const uint8_t *question, 
    uint8_t answer[4], 
    const uint8_t answerConfig
){
    // We'll need a way to map the configuration mux to the array of bits.
    const auto bitArray = WDT_QA_Generate_Question_Bit_Array(answerConfig);
 ...
 */
IResult<>
TCAN114X::WDT_Config_Write(const WatchdogConfig & WDTConfig){
 ...
 * @returns IResult<> if the write was successful or not, based on if TCAN114X_CONFIGURE_VERIFY_WRITES is defined in tcan114x.h
 */
IResult<TCAN114X::WatchdogConfig>
TCAN114X::WDT_Config_Read(){
    WatchdogConfig WDTConfig;

    if(const auto res = read_reg(REG_TCAN114X_WD_CONFIG_1, &WDTConfig.word_config_1);
        res.is_err()) return Err(res.unwrap_err());

    if(const auto res = read_reg(REG_TCAN114X_WD_CONFIG_2, &WDTConfig.word_config_2);
        res.is_err()) return Err(res.unwrap_err());

    if(const auto res = read_reg(REG_TCAN114X_WD_RST_PULSE, &WDTConfig.word_wd_rst_pulse);
        res.is_err()) return Err(res.unwrap_err());
 ...
 */
IResult<>
TCAN114X::WDT_Config_QA_Write(const Watchdog_QA_Config & QAConfig){
 ...
 * @returns IResult<> if the write was successful or not, based on if TCAN114X_CONFIGURE_VERIFY_WRITES is defined in tcan114x.h
 */
IResult<TCAN114X::Watchdog_QA_Config>
TCAN114X::WDT_Config_QA_Read(){
    Watchdog_QA_Config QAConfig;
    if(const auto res = read_reg(REG_TCAN114X_WD_QA_CONFIG, &QAConfig.word_qa_config);
        res.is_err()) return Err(res.unwrap_err());
    return Ok(QAConfig);
}
