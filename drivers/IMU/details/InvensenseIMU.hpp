...
public:
    ...
    [[nodiscard]] Result<void, Error> write_reg(const uint8_t addr, const uint8_t data) {
        if(i2c_drv_){
            if(const auto res = i2c_drv_->write_reg(uint8_t(addr), data); res.is_err())
                return Err(res.unwrap_err());
            return Ok();
        }else if(spi_drv_){
            if(const auto res = spi_drv_->write_single<uint8_t>(uint8_t(addr & 0x7f), CONT);
    ...
    [[nodiscard]] Result<void, Error> read_reg(const uint8_t addr, uint8_t & data) {
        if(i2c_drv_){
            if(const auto res = i2c_drv_->read_reg(uint8_t(addr), data); res.is_err())
                return Err(res.unwrap_err());
            return Ok();
        }else if(spi_drv_){
            if(const auto res = spi_drv_->write_single<uint8_t>(uint8_t(addr & 0x7f), CONT);
...
public:
    InvensenseSensor_Phy(const hal::I2cDrv & i2c_drv):
        i2c_drv_(i2c_drv){;}
    InvensenseSensor_Phy(hal::I2cDrv && i2c_drv):
        i2c_drv_(std::move(i2c_drv)){;}
    InvensenseSensor_Phy(Some<hal::I2c *> i2c, const hal::I2cSlaveAddr<7> addr):
        i2c_drv_(hal::I2cDrv{i2c, addr}){;}
    InvensenseSensor_Phy(const hal::SpiDrv & spi_drv):
        spi_drv_(spi_drv){;}
