...
public:
    ...
    StmicroImu_Phy(Some<hal::I2c *> i2c, const hal::I2cSlaveAddr<7> addr):i2c_drv_(hal::I2cDrv{i2c, addr}){;}
    StmicroImu_Phy(const hal::SpiDrv & spi_drv):spi_drv_(spi_drv){;}
    StmicroImu_Phy(hal::SpiDrv && spi_drv):spi_drv_(spi_drv){;}
    StmicroImu_Phy(Some<hal::Spi *> spi, const hal::SpiSlaveIndex index):spi_drv_(hal::SpiDrv{spi, index}){;}
