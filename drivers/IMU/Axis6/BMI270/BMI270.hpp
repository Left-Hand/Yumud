...
#ifdef BMI270_DEBUG
#undef BMI270_DEBUG
#define BMI270_DEBUG(...) DEBUG_PRINTLN(SpecToken::Space, std::hex, ##__VA_ARGS__, "\t|", __PRETTY_FUNCTION__);
#else
#define BMI270_DEBUG(...)
#endif
...
public:
    BMI270(const I2cDrv & i2c_drv):i2c_drv_(i2c_drv){;}
    BMI270(I2cDrv && i2c_drv):i2c_drv_(std::move(i2c_drv)){;}
    BMI270(I2c & i2c, const uint8_t i2c_addr = default_i2c_addr):i2c_drv_(I2cDrv{i2c, default_i2c_addr}){;}

    BMI270(const SpiDrv & spi_drv):spi_drv_(spi_drv){;}
    BMI270(SpiDrv && spi_drv):spi_drv_(std::move(spi_drv)){;}
    BMI270(Spi & spi, const uint8_t index):spi_drv_(SpiDrv{spi, index}){;}
