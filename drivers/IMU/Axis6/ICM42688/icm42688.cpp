...
void ICM42688::init(){
	/*指定Bank0*/
	phy_.write_reg(0x76,0x00);
	/*软重启*/
	phy_.write_reg(0x11,0x01);
	delay(30);
	/*读取中断位 切换SPI*/
//	buf = IMU->ReadReg(0x2D);
	...
	phy_.write_reg(0x76,0x00);
	/*Gyr设置*/
	phy_.write_reg(0x4F,0x06);//2000dps 1KHz
	/*Acc设置*/
	phy_.write_reg(0x50,0x06);//16G 1KHz
	/*电源管理*/
	phy_.write_reg(0x4E,0x0F);//ACC GYR LowNoise Mode
	...
	phy_.write_reg(0x76,0x00);
	/*中断输出设置*/
//	phy_.write_reg(0x14,0x12);//INT1 INT2 脉冲模式，低有效
	/*Gyr设置*/
	phy_.write_reg(0x4F,0x06);//2000dps 1KHz
	/*Acc设置*/
	phy_.write_reg(0x50,0x06);//16G 1KHz
	...
	/*Tem设置&Gyr_Config1*/
	phy_.write_reg(0x51,0x56);//BW 82Hz Latency = 2ms
	/*GYR_ACC_CONFIG0*/
	phy_.write_reg(0x52,0x11);//1BW
	/*ACC_CONFIG1*/
	phy_.write_reg(0x53,0x0D);//Null
	/*INT_CONFIG0*/
	phy_.write_reg(0x63,0x00);//Null
	/*INT_CONFIG1*/
//	phy_.write_reg(0x64,0x00);//中断引脚正常启用
	/*INT_SOURCE0*/
	phy_.write_reg(0x65,0x08);//DRDY INT1
	/*INT_SOURCE1*/
	phy_.write_reg(0x66,0x00);//Null
	/*INT_SOURCE3*/
	phy_.write_reg(0x68,0x00);//Null
	/*INT_SOURCE3*/
	phy_.write_reg(0x69,0x00);//Null
	
/*****抗混叠滤波器@536Hz*****/
	
	/*GYR抗混叠滤波器配置*/
	/*指定Bank1*/
	phy_.write_reg(0x76,0x01);
	/*GYR抗混叠滤波器配置*/
	phy_.write_reg(0x0B,0xA0);//开启抗混叠和陷波滤波器
	phy_.write_reg(0x0C,0x0C);//GYR_AAF_DELT 12 (default 13)
	phy_.write_reg(0x0D,0x90);//GYR_AAF_DELTSQR 144 (default 170)
	phy_.write_reg(0x0E,0x80);//GYR_AAF_BITSHIFT 8 (default 8)
	
	/*ACC抗混叠滤波器配置*/
	/*指定Bank2*/
	phy_.write_reg(0x76,0x02);
	/*ACC抗混叠滤波器配置*/
	phy_.write_reg(0x03,0x18);//开启滤波器 ACC_AFF_DELT 12 (default 24)
	phy_.write_reg(0x04,0x90);//ACC_AFF_DELTSQR 144 (default 64)
	phy_.write_reg(0x05,0x80);//ACC_AAF_BITSHIFT 8 (default 6)
...
/*****自定义滤波器1号@111Hz*****/
	...
	phy_.write_reg(0x76,0x00);
	/*滤波器顺序*/
	phy_.write_reg(0x51,0x12);//GYR滤波器1st
	phy_.write_reg(0x53,0x05);//ACC滤波器1st
	/*滤波器设置*/
	phy_.write_reg(0x52,0x33);//111Hz 03
	/*指定Bank0*/
	phy_.write_reg(0x76,0x00);
	/*电源管理*/
	phy_.write_reg(0x4E,0x0F);//ACC GYR LowNoise Mode
