...
private:
    ...
    [[nodiscard]] IResult<> write_reg(const RegCopy<T> & reg){
        if(const auto res = i2c_drv_.write_reg(uint8_t(T::ADDRESS), reg.to_bits(), std::endian::big);
    ...
    [[nodiscard]] IResult<> read_reg(T & reg){
        if(const auto res = i2c_drv_.read_reg(uint8_t(T::ADDRESS), reg.as_bits_mut(), std::endian::big);
            res.is_err()) return Err(res.unwrap_err());
        return Ok();
    }
