...
public:
    ...
    IResult<> set_by_mask(const hal::PinMask mask) {
        buf_mask_ = buf_mask_ | mask;
        return write_by_mask(buf_mask_);
    }

    IResult<> clr_by_mask(const hal::PinMask mask) {
        buf_mask_ = buf_mask_ & (~mask);
        return write_by_mask(buf_mask_);
    }
    ...
    IResult<BoolLevel> read_nth(const Nth nth) ;
    ...
    IResult<> enable_led_mode(const hal::PinMask pin);

    IResult<> set_led_current_limit(const CurrentLimit limit);

    IResult<> set_led_current_dutycycle(
