...
/**
  ...
  *         - SD_RESPONSE_NO_ERROR: Sequence succeed
  */
SD_Error SD_read_bulkBlocks(uint8_t* pBuffer, uint64_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
...
/**
  ...
  *         - SD_RESPONSE_NO_ERROR: Sequence succeed
  */
SD_Error SD_write_bulkBlocks(uint8_t* pBuffer, uint64_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
	uint32_t i = 0, Offset = 0;
	SD_Error rvalue = SD_RESPONSE_FAILURE;
