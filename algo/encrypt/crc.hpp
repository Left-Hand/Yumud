...
__fast_inline constexpr T calc_crc(T remainder, uint8_t value)
{
...
template<typename T, unsigned POLYNOMIAL>
__fast_inline constexpr  T calc_crc(T remainder, const uint8_t *buffer, size_t length)
{
    while (length--)
...
__fast_inline constexpr  uint8_t calc_crc8(uint8_t remainder, uint8_t value)
{
    return calc_crc<uint8_t, POLYNOMIAL>(remainder, value);
}

template<unsigned POLYNOMIAL>
__fast_inline constexpr  uint16_t calc_crc16(uint16_t remainder, uint8_t value)
{
    return calc_crc<uint16_t, POLYNOMIAL>(remainder, value);
}

template<unsigned POLYNOMIAL>
__fast_inline constexpr  uint8_t calc_crc8(uint8_t remainder, const uint8_t *buffer, size_t length)
{
